...
Image2rtsp::Image2rtsp() : Node("image2rtsp"){
    ...
    this->declare_parameter("topic_compressed", "/color/image_compressed");
    ...
    this->declare_parameter("caps_3", "video/x-compressed, framerate =");
    ...
    topic_compressed = this->get_parameter("topic_compressed").as_string();
    ...
    caps_3 = this->get_parameter("caps_3").as_string();
    ...
    subscriptionCompressed_ = this->create_subscription<sensor_msgs::msg::CompressedImage>(topic_compressed, 10, std::bind(&Image2rtsp::topic_compressed_callback, this, _1));
    ...
    pipeline_tail = "key-int-max=30 ! video/x-h264, profile=baseline ! rtph264pay name=pay0 pt=96 )";
    if (camera == false){
        pipeline_head = "( appsrcComp name=imagesrc do-timestamp=true min-latency=0 max-latency=0 max-bytes=1000 is-live=true ! videoconvert ! videoscale ! ";
        pipeline = pipeline_head + caps_3 + framerate + caps_2 + " ! x264enc tune=zerolatency bitrate=" + bitrate + pipeline_tail;
        rtsp_server_add_url(mountpoint.c_str(), pipeline.c_str(), (GstElement **)&(appsrcComp));
    }
    else {
        pipeline = "( " + source + " ! videoconvert ! videoscale ! " + caps_3 + framerate + caps_2 + " ! x264enc tune=zerolatency bitrate=" + bitrate + pipeline_tail;
        rtsp_server_add_url(mountpoint.c_str(), pipeline.c_str(), NULL);
    }
    RCLCPP_INFO(this->get_logger(), "Stream available at rtsp://%s:%s%s", gst_rtsp_server_get_address(rtsp_server), port.c_str(), mountpoint.c_str());
